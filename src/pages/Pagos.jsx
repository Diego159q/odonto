...
import { buildWhatsAppUrl, downloadPaymentReceiptPdf, getPatientPhoneFrom, paymentReminderMessage } from '../utils/noApiAutomation';
...
const ViewModal = ({ pago, onClose }) => <BaseModal title="Detalle de pago" onClose={onClose}><div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><Info label="Paciente" value={getPatientName(pago)} /><Info label="Fecha" value={formatDate(pago.fecha)} /><Info label="Monto" value={formatCurrency(pago.montoPagado || pago.monto)} /><Info label="Metodo" value={pago.metodoPago || '-'} /><Info label="Operacion" value={pago.numeroOperacion || '-'} /><Info label="Observaciones" value={pago.observaciones || '-'} /></div><div className="flex flex-wrap justify-end gap-2 mt-6"><button onClick={() => window.open(buildWhatsAppUrl({ phone: getPatientPhoneFrom(pago), message: paymentReminderMessage(pago) }), '_blank', 'noopener,noreferrer')} className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-2 text-sm font-bold text-emerald-300 hover:bg-emerald-500/20">WhatsApp saldo</button><button onClick={() => downloadPaymentReceiptPdf(pago)} className="rounded-xl border border-blue-500/30 bg-blue-500/10 px-4 py-2 text-sm font-bold text-blue-300 hover:bg-blue-500/20">Recibo PDF</button><button onClick={onClose} className="rounded-xl border border-slate-700 bg-slate-800 px-4 py-2 text-sm font-bold text-slate-200 hover:bg-slate-700">Cerrar</button></div></BaseModal>;
